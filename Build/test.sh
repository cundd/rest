...
function check_phpunit_path_for_functional_tests() {
    $(get_phpunit_path_for_functional_tests > /dev/null) || {
        print_error "Could not find phpunit to run functional tests";
        exit 1;
    };
}
...
function check_phpunit_path_for_unit_tests() {
    $(get_phpunit_path_for_unit_tests &> /dev/null) || {
        print_error "Could not find phpunit to run unit tests";
        exit 1;
    };
}
...
function check_mysql_credentials {
    print_debug "Check MySQL credentials";
    php -r '@mysqli_connect("'${typo3DatabaseHost}'", "'${typo3DatabaseUsername}'", "'${typo3DatabasePassword}'", "'${typo3DatabaseName}'", "'${typo3DatabasePort}'") or exit(1);' || {
        print_error "Could not connect to MySQL ($typo3DatabaseUsername:$typo3DatabasePassword@$typo3DatabaseHost:$typo3DatabasePort / $typo3DatabaseName)";
    }
}
...
function init_database {
    ...
    print_info "Connect to database '$typo3DatabaseName' at '$typo3DatabaseHost:$typo3DatabasePort' using '$typo3DatabaseUsername' '$typo3DatabasePassword'";
}

# Prepare the TYPO3 system
function init_typo3 {
	local baseDir=`pwd`;
    print_debug "Check for phpunit at ${TYPO3_PATH_WEB}/bin/phpunit";
	if [[ ! -x ${TYPO3_PATH_WEB}/bin/phpunit ]]; then
        print_debug "Check for composer.json in '$TYPO3_PATH_WEB'";
		if [[ -f "$TYPO3_PATH_WEB/composer.json" ]]; then
		    cd ${TYPO3_PATH_WEB};
            print_debug "Run composer install in '$TYPO3_PATH_WEB'";
            lib::composer install;

            print_debug "Go back into '$baseDir'";
            cd ${baseDir};
		fi
	fi
}
...
function init_typo3_path_web {
    if [ "${TYPO3_PATH_WEB}" == "" ]; then
        ...
        print_debug "TYPO3_PATH_WEB is '$TYPO3_PATH_WEB'";
    fi;
}
